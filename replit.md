...
- **Email Access:** Uses existing project inboxes (no separate AgentMail inbox needed)
...
- **Assistant Workspace:** Dedicated interface at `/assistant-workspace` for Email Assistant agents to monitor and respond to emails across all projects. Aggregates email threads from all project inboxes (businessblueprint@agentmail.triadblue.com, hostsblue@agentmail.triadblue.com, etc.) into unified view. Supports reading messages, replying with attachments (max 10 files, 10MB each), and voice input for hands-free operation. Separate from main Email Chat page to enable clean organization and searchability for both platform builders and email assistants. No additional AgentMail inboxes required - reuses existing project email infrastructure.
