...
- **Speech-to-Text Voice Input:** Browser-based Web Speech API integration for hands-free input across the application. Reusable VoiceInput component (icon button) and VoiceInputButton (text button with recording states) support both single-utterance and continuous recording modes. Integrated into Agent Chat message input, Task creation modal (title and description fields), and Conversations page (all text inputs). Features real-time interim transcript display, error handling for microphone permissions, and automatic text appending to existing input. No external API keys required - uses native browser capabilities.
